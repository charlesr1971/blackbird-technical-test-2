...
function draw() {
  ...
  line(0, hght, wdth * 3, hght);
  line(0, hght * 2, wdth * 3, hght * 2);
  for (var a = 0; a < 3; a++) {
	for (var b = 0; b < 3; b++) {
	  let x = wdth * b + wdth / 2;
	  let y = hght * a + hght / 2;
...
function resultCheck() {
  ...
  if (matches(tictactoeCanvas[0][0], tictactoeCanvas[1][1], tictactoeCanvas[2][1])) {
...
function mousePressed () {
  const x = floor(mouseX / wdth);
  const y = floor(mouseY / hght);
